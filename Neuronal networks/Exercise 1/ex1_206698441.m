function ex1_206698441(rrr)
    ...
    while (errors_exist && turn<MAX_TURNS)
        ...
        for sample_ind=1:length(x1)
            ...
            net_input = sum(w.*input_vec)+1*bias;
            ...
            if (output~=t(sample_ind))
                ...
                w = w + input_vec*learningRate*(t(sample_ind)-output);
    ...
    function y=calc_y(x,w,bias)
        % This funtcion calculates the Y value for the input
        % x value, assuming x and y are on the line separating the 
        % two classes using the explicit line equation.
        y = (-w(1)*x-bias)/w(2);
    end
end
